...
import (
	...
	"pdx-chain/params"
	whisper "pdx-chain/whisper/whisperv6"
)
...
var (
	...
	UtopiaBlockDelayFlag = cli.IntFlag{
		Name:  "BlockDelay",
		Usage: "block miner and wait time",
		Value: 1000,
	}
	UtopiaNumMastersFlag = cli.IntFlag{
		Name:  "NumMasters",
		Usage: "Masters total number of",
		Value: 10,
	}
	UtopiaChaincodeRpcPortFlag = cli.IntFlag{
		Name:  "ccRpcPort",
		Usage: "Utopia chaincode rpc port",
		Value: 1234,
	}
	UtopiaIaasServerFlag = cli.StringFlag{
		Name:  "iaasServer",
		Usage: "iaas server",
		Value: "http://10.0.0.161:8080",
	}
	UtopiaGasLessFlag = cli.BoolFlag{
		Name:  "gasless",
		Usage: "gas free for Chain Alliance",
	}
	UtopiaWaitTime = cli.IntFlag{
		Name:  "waitTime",
		Usage: "waitTime",
		Value: 0,
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `pdxc account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	log.Info("Maximum peer count", "PDX", ethPeers, "LES", lightPeers, "total", cfg.MaxPeers)
...
func RegisterEthService(stack *node.Node, cfg *eth.Config) {
	...
	if cfg.SyncMode == downloader.LightSync {
		...
		err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			fullNode, err := eth.New(ctx, cfg, stack)
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "pdxc.", map[string]string{
				"host": hosttag,
			})
		}
	}
...
// new format.
//
// e.g. pdxc account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// pdxc --keystore /tmp/mykeystore --lightkdf account new
//
