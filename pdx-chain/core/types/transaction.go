...
//verify cert
func (tx *Transaction) VerifyCert(category int) (ok bool) {
	//get cert
	var (
		tcAdmCrt string
		err      error
	...
	txPubkey := hex.EncodeToString(tx.PublicKey())
	certPubkey := hex.EncodeToString(pubkey)
	if txPubkey != certPubkey {
		log.Error("cert public key no match", "tx pubkey", txPubkey, "certPubkey", certPubkey)
		return
	}

	//save cert for check
	BaapHome := os.Getenv("PDX_BAAP_HOME")
	f, err := os.OpenFile(BaapHome+"/conf/tcadm.crt", os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0666)
	defer f.Close()
	if err != nil {
		log.Error("open error", "err", err)
		return
	}
	_, err = f.Write([]byte(tcAdmCrt))
	if err != nil {
	...
	if err != nil {
		log.Error("command error", "err", err)
		return
	}
