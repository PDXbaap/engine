...
func run(evm *EVM, contract *Contract, input []byte, extra map[string][]byte) ([]byte, error) {
	if contract.CodeAddr != nil {
		...
		if pdxcc.CanExec(contract.CodeAddr.Hex()) {
			return RunPrecompiledContract(evm, precompiles[util.EthAddress("baap-deploy")], input, contract, extra)
		}
...
func (evm *EVM) Call(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int,
	...
	if !evm.StateDB.Exist(addr) {
		...
		if precompiles[addr] == nil && !pdxcc.CanExec(addr.String()) && evm.ChainConfig().IsEIP158(evm.BlockNumber) && value.Sign() == 0 && !utils.IsFreeGas() {
	...
	//log.Info(fmt.Sprintf("evm run s contract --- %s(%d)", txid, contract.Gas))
...
func (evm *EVM) create(caller ContractRef, code []byte, gas uint64, value *big.Int, address common.Address) ([]byte, common.Address, uint64, error) {
	...
	ret, err := run(evm, contract, nil, make(map[string][]byte))
