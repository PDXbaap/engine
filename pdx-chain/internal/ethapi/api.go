...
func (s *PublicBlockChainAPI) doCall(ctx context.Context, args CallArgs, blockNr rpc.BlockNumber, vmCfg vm.Config, timeout time.Duration) ([]byte, uint64, bool, error) {
	...
	res, gas, failed, err := core.ApplyMessage(evm, msg, gp, make(map[string][]byte))
...
func (s *PublicTransactionPoolAPI) CheckTxCompletedByHash(ctx context.Context, from common.Address, txHash common.Hash) bool {
	...
	keyHash := utils.DepositKeyHash(from, txHash)
...
func (s *PublicTransactionPoolAPI) GetUncompletedTransactionByAddress(ctx context.Context, from common.Address) []common.Hash {
	...
	//withdrawTxAddr := utils.GenWithdrawTxAddr(from)
	////withdrawTxAddr := utopia.Keccak256ToAddress("x-chain-transfer-withdraw")
	//log.Info("withdrawTxAddr", "is", withdrawTxAddr)
	//stateDB.ForEachPDXStorage(withdrawTxAddr, func(txhash common.Hash, value []byte) bool {
	//	log.Info("ForEachPDXStorage txhash", "hash", txhash.String())
	//	var withdraw vm.Withdraw
	//	err = json.Unmarshal(value, &withdraw)
	//	if err != nil {
	//		log.Error("unmarshal withdraw error", "err", err)
	//		return false
	//	}
	//	log.Info("ForEachPDXStorage withdraw info", "is", withdraw)
	//	host := iaasconn.GetNodeFromIaas(withdraw.DstChainID)
	//	if host != "" {
	...
	//			return false
	//		}
	//		if !cli.CheckTxCompletedByHash(from, txhash) {
	...
	for _, v := range txs {
		...
		host := iaasconn.GetNodeFromIaas(v.Payload.DstChainID)
		if host != "" {
			cli, err := client.Connect(host)
			if err != nil {
				log.Error("client connect error", "err", err)
...
func (s *PublicTransactionPoolAPI) GetWithDrawTransactionByHash(ctx context.Context, hash common.Hash) *WithdrawRPCTransaction {
	// Try to return an already finalized transaction
	...
	if tx, blockHash, blockNumber, index := rawdb.ReadTransaction(s.b.ChainDb(), hash); tx != nil {
		...
		withdrawTx.Status = -1
		//stateDB.ForEachPDXStorage(withdrawTxAddr, func(txhash common.Hash, value []byte) bool {
		//	log.Info("ForEachPDXStorage txhash", "hash", txhash.String(), "res", res.String())
		//	if txhash == hash {
		//		log.Info("ForEachPDXStorage txhash success 1")
		//		withdrawTx.Status = 1
		//		if res.Cmp(big.NewInt(utopia.XChainTransferCommitNum)) == +1 {
		//			log.Info("ForEachPDXStorage txhash success 2")
		//			withdrawTx.Status = 2
		//		}
		//		return true
		//	}
...
func (s *PublicTransactionPoolAPI) BaapQuery(ctx context.Context, encodedTx hexutil.Bytes) (string, error) {
	return pdxcc.BaapQuery(encodedTx)
}
...
func Filter(tx *types.Transaction) error {
	if tx.GasPrice().Cmp(big.NewInt(0)) <= 0 {
		return errors.New("gas price too low")
	}

	switch {
	case utils.IsBaapTrustTree(tx.To()): //trust tx white list
		pub := tx.PublicKey()
		key, _ := crypto.DecompressPubkey(pub)
		from := crypto.PubkeyToAddress(*key)
	...
	case utils.IsDeposit(tx.To()):
		log.Info("检测黑名单")
		err := blacklist.BlackListTxTask.VerifyBlackListTx(tx) //判断是否在黑名单
		if err != nil {
			log.Info("拦截成功！", "err", err)
			return err
		}
	case utils.IsTcUpdater(tx.To()): //verify cert
		if !tx.VerifyCert(1) {
			return errors.New("cert illegal")
		}
	case utils.IsTrustTxWhiteList(tx.To()):
		if !tx.VerifyCert(2) {
			return errors.New("cert illegal")
		}
