...
func MakeChaincodeArgs(fcn string, params [][]byte) [][]byte {
	bargs := make([][]byte, len(params)+1)
...
func EthHash(value []byte) common.Hash {
	return common.BytesToHash(crypto.Keccak256(value))
}

func EthAddress(namespace string) common.Address {
	return common.BytesToAddress(crypto.Keccak256([]byte(namespace))[12:])
}
