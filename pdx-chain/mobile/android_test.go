...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "pdxc-android-")
	if err != nil {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "pdxc-chain/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "pdxc.aar"), "pdxc.aar", os.ModePerm)
