...
type LightPeer interface {
	...
	RequestCommitsByNumber(uint64, int, int, bool) error
	RequestLatest() error
}
...
func (w *lightPeerWrapper) RequestCommitsByNumber(i uint64, amount int, skip int, reverse bool) error {
	return nil
}
func (w *lightPeerWrapper) RequestLatest() error {
	return nil
}
...
func (ps *peerSet) SubscribeAssertionNewFeed(ch chan<- *p2p.Peer) event.Subscription {
	return ps.assertionNewFeed.Subscribe(ch)
}
