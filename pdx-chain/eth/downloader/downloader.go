...
type Downloader struct {
	...
	AssertionNewPeer chan *p2p.Peer
...
func New(mode SyncMode, stateDb ethdb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	...
	dl := &Downloader{
		...
		AssertionNewPeer:          make(chan *p2p.Peer,1024),
	...
	dl.peers.SubscribeAssertionNewFeed(dl.AssertionNewPeer)
	go dl.qosTuner()
	go dl.stateFetcher()
...
func (d *Downloader) FindCommitAncestorp(id string) (uint64, error) {
	...
	origin, err := d.findCommitAncestor(p, height)
	if err != nil {
		return 0, err
	}
	return origin, nil
...
func (d *Downloader) SynchroniseCommit(id string) error {
	if !atomic.CompareAndSwapInt32(&d.commitSynchronising, 0, 1) {
		return errBusy
	}
	defer atomic.StoreInt32(&d.commitSynchronising, 0)
	...
	pivot := uint64(0)
	go d.processCommits()
	return d.fetchCommits(p, origin+1, pivot)
}
...
func (d *Downloader) SyncNormalDeleteCommit(origin *types.Block) bool {
	...
	if originCommitNum >= currentCommitNum || currentCommitNum-originCommitNum <= 1 {
		log.Debug("originCommitNum >= currentCommitNum || currentCommitNum-originCommitNum <= 1 ||originCommitNum==0")
		return false
	}
...
func (d *Downloader) fetchCommitHeight(p *peerConnection) (*types.Block, error) {
	...
	for {
		...
		case <-timeout:
			p.log.Debug("Waiting for head commit timed out", "elapsed", ttl)
			return nil, errTimeout
		}
	}
}
...
func (d *Downloader) findCommitAncestor(p *peerConnection, height uint64) (uint64, error) {
	// Figure out the valid ancestor range to prevent rewrite attacks
		...
		case packet := <-d.commitCh:
			...
			for i := len(headers) - 1; i >= 0; i-- {
				...
				if d.Commitchain.HasBlock(headers[i].Hash(), headers[i].NumberU64()) {
					number, hash = headers[i].NumberU64(), headers[i].Hash()
	...
	if hash != (common.Hash{}) {
		...
		p.log.Debug("Found common commit ancestor", "number", number, "hash", hash)
		return number, nil
	}
	// Ancestor not found, we need to binary search over our chain
	start, end := uint64(0), head
	...
	p.log.Debug("Found common commit ancestor", "number", start, "hash", hash)
	return start, nil
}
...
//PDX
func (d *Downloader) fetchCommits(p *peerConnection, from uint64, pivot uint64) error {
	p.log.Debug("Directing commit downloads", "origin", from)
	defer p.log.Debug("commit download terminated")
	...
	for {
		...
		case packet := <-d.commitCh:
			...
			// Insert all the new headers and fetch the next batch
			if len(headers) > 0 {
				p.log.Trace("Scheduling new commits", "count", len(headers), "from", from)
...
//PDX
func (d *Downloader) processCommits() error {
	for {
		select {
		case blocks := <-d.processCommitCh:
			...
			for _, block := range blocks {
				...
				currentHeight := d.blockchain.CurrentBlock().Number()
				if commitExtra.NewBlockHeight.Cmp(currentHeight) > 0 {
					types2.SyncCommit.Store(commitExtra.NewBlockHeight.Uint64())
					timer := time.NewTimer(time.Minute * 10)
					select {
					case <-types2.SyncCommitCh:
					case <-timer.C:
					}
				}
				d.Worker.ProcessCommitBlock(block, true)
			}
			if len(d.processCommitCh) == 0 {
				d.CbFetcher.Synced <- struct{}{}
				return nil
			}
		}
	}
...
//PDX
func (d *Downloader) ProcessCommitsAssociated() error {
	for {
		select {
		...
		Associated:
			...
			log.Info("ProcessCommitsAssociated半生保存完成")
		}
	}
}
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	var b bool
	//删除commit
	if examineSync.ExamineBlockWork==0{ //同步完毕之后,在进行commit调整

		b = d.SyncNormalDeleteCommit(blocks[0])
	}
