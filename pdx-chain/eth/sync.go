	...
	for {
		...
		case <-pm.newPeerCh:
			...
			if pm.blockchain.CurrentBlock().NumberU64()<=1{
				...
				go pm.synchronise(pm.peers.SyncPeer(),0)
			}

		case <-forceSync.C:
			// Force a sync even if not enough peers are present
			if !engine.IslandState.Load().(bool) {
				log.Info("时间到了,开始同步")
				go pm.synchronise(pm.peers.SyncPeer(), 0)
			}
...
func (pm *ProtocolManager) synchronise(peer *peer, normalNum uint64) {
	...
	log.Debug("节点信息", "peerCommit高度", peer.CommitNum, "本地commit高度", localCommitNum,
		"peerNormal高度", normalNum, "本地Normal高度", localNormalNum)

	if normalNum != 0 && peer.CommitNum <= localCommitNum && normalNum <= localNormalNum {
...
func (pm *ProtocolManager) synchroniseCommit(peer *peer) {
	if peer == nil {
		log.Error("commit sync return : peer is nil")
		return
	}

	if err := pm.downloader.SynchroniseCommit(peer.id); err != nil {
